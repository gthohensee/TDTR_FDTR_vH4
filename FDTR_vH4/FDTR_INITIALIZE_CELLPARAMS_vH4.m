%FDTR_INITIALIZE_CELLPARAMS_vH4 - Unpacks cell arrays into variables for the
...
%
% Outputs
%    f_data  - COLUMN VECTOR of frequency data
...
%
%
%    Zind    - Closest index of f_data such that Zdelay = tdelay(Zind).
%              Zdelay is user-specified time delay (ps) for shortest time
%              delay from which to evaluate goodness-of-fit.
%    sigfit  - 1 == fitting V(in), 2 == fitting V(out), 3 == amplitude, 4 == phase, otherwise ratio.
...
%
% See also: The FDTR_FIT_vH4.m, FDTR_MANUALFIT_vH4.m
...
%                              retitled vH3.
%                   18-Aug-2015 - amplitude compatibility, sigfit = 3.
...
% data parameters
%datparams = {f_data ratio_data datadir offset} or {tdelay Vin_data datadir offset}
if PL(1) < 1, error('no frequency vector'); else f_data = datparams{1}; end
if PL(1) < 2, error('no data vector'); 
else
    switch sigfit
        case 1, Vin_data = datparams{2};
        case 2, Vout_data = datparams{2};
        case 3, Amp_data = datparams{2};
        case 4, phi_data = datparams{2};
