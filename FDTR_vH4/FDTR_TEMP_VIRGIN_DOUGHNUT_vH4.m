function Integrand=FDTR_TEMP_VIRGIN_DOUGHNUT_vH4(kvectin,freq,LCTE,r_pump,r_probe,A_pump,xoffset)
...
%
% Virgin versus probed doughnut: Virgin gives the real temperature 
% distribution, probed gives the temperature sampled by a Gaussian probe.
%
% This program is vectorized/optimized to handle all frequencies 
% simultaneously (f is a ROW vector)
%
% Syntax:  [Integrand,G]=FDTR_TEMP_VIRGIN_DOUGHNUT_vH4(kvectin,freq,LCTE,r_pump,r_probe,A_pump,xoffset)
%
% Inputs:
...
%% Virgin versus probed doughnut: Virgin gives the real temperature 
% distribution, probed gives the temperature sampled by a Gaussian probe beam.
% S = GetHankel_OffsetBeam(kvect,xoffset,r_probe,1); % probed doughnut
S = besselj(0,2*pi*kvect*xoffset);               % virgin doughnut
%%
