function [Z,deltaR_model,ratio_model,LCTE,T_adj]=...
    FDTR_FIT_vH4(X, Xij, datparams,sysparams,calparams,matparams,Tparams)
%FDTR_FIT_vH4 - automatically fit thermal model to TDTR ratio data.
...
%    TDTR_FIT_vH4(X, Xij, datparams,sysparams,calparams,matparams,Tparams)
%
% Inputs:
...
%
% Example:
%    Xsol = fminsearch(@(X),TDTR_FIT_vH4(X,Xij,...
%                   datparams,sysparams,calparams,matparams,Tparams),...
%                   Xguess,options);
%
% Other m-files required: TDTR_REFL_vH4.m, FDTR_TEMP_vH4.m,
...
if T0 ~= -1
    error('FDTR thermal model not yet prepared for self-consistent steady-state heating');
...
if doughnut && sigfit ~= 2, error('Code only designed for beam offset fit to V(out).'); end          
switch sigfit
    case 1 % V(in) fit
        % Construct normalized V(in) model and data,
        % relative to its value at ZF picoseconds.
    ...
    case 2 % V(out) fit
        if doughnut
            ...
            % relative to its mean value near ZF frequency
    ...
    case 4
        %phi_model is phase delay, degrees
    otherwise % ratio fit
        % do nothing here
end
...
if doughnut
    ...
    [deltaR_modelx, ratio_modelx]=FDTR_REFL_vH4(matparams,sysparams,...
                                               A_pump,intscheme,nnodes,xvect);
    % deltaR and ratio may be a matrix (frequency, offset). Pull out
...
figure(10)
loglog(f/1e6,ratio_data,'ob',f/1e6,ratio_model,'r-'); 
xlabel('Frequency (MHz)','FontSize',16);

switch sigfit
    case 1 % V(in)
        ylabel('normalized V(in)','FontSize',16);
        axis([min(f/1e6) 2*max(f/1e6) min([nVin_data;nVin_model])/2 max([nVin_data;nVin_model])*2])
    case 2
        ylabel('normalized V(in)','FontSize',16);
        axis([min(f/1e6) 2*max(f/1e6) min([nVout_data;nVin_model])/2 max([nVout_data;nVin_model])*2])
    case 3
        loglog(f/1e6,nAmp_data,'ob',f/1e6,nAmp_model,'r');

        ylabel('normalized amplitude','FontSize',20);
        axis([min(f/1e6) 2*max(f/1e6) min([nAmp_data;nAmp_model])/2 1.4])
    case 4
        loglog(f/1e6,phi_data,'ob',f/1e6,phi_model,'r');

        ylabel('Phase delay (degrees)','FontSize',20);
        axis([min(f/1e6) 2*max(f/1e6) -180 180])
    otherwise % ratio fit
        ylabel('Ratio','FontSize',16); 
        axis([min(f/1e6) 2*max(f/1e6) min([ratio_data;ratio_model])/2 max([ratio_data;ratio_model])*2])
end
...
%% goodness-of-fit Z from fractional residuals
    switch sigfit
        ...
        % residuals by the amount of frequency away from ZF, 
