%FDTR_MAIN_vH4 - Given system & thermal parameters, do thermal modeling.
...
%    sigfit          - 1 if fitting V(in), 2 if fitting V(out), 3 for
%                      amplitude sqrt(Vin^2 + Vout^2), any other
...
%
% Other m-files required: TDTR_REFL_vH4.m, TDTR_FIT_vH4.m,
%                         TDTR_MANUALFIT_vH4.m, TDTR_TDEP_vH4.m,
%                         SS_Heating_vH4.m, TDTR_REFL_vH4.m,
%                         TDTR_TEMP_vH4.m, INITIALIZE_CELLPARAMS_vH4.m,
%                         senseplot_vH4.m, errorbars_vH4.m,
%                         errorbar_conditions_vH4.m, genLCTEtext.m,
%                         extract_interior.m, rombint_VV3.m, SimpsonInt.m,
%                         lgwt_V4.m, mtimesx package,
%                         TDTR_TEMP_DOUGHNUT_vH4.m, GetHankel_OffsetBeam.m
% Subfunctions: none
% MAT-files required: none
%
% See also: The TDTR_TTM_vH4 package
...
%                               errorbar.
%                   2/17/2015 - vH3.
%                   8/18/2015 - Amplitude fitting, sigfit = 3.
%                   9/2/2015  - adapted to FDTR.
%                   7/11/2016 - vH4.
%------------- BEGIN CODE --------------
%%
...
consider_error = 0;           % MAIN does not do error bars, errorbar_vH4 does.
LCTE_err = zeros(size(LCTE)); % MAIN does not do error bars, errorbar_vH4 does.
...
if senseplot
    ...
    FDTR_senseplot_offset_vH4(datparams,sysparams, calparams, matparams, Tparams); 
    toc
...
if ebar
    ...
    % errorbars_vH4 calls FIT, requires all cellparams
    [kErr_perc, kErr_abs, ...
        ErrSummary_perc, ErrSummary_abs] = ...
        FDTR_errorbars_vH4(Xijc,datparams,sysparams,...
...
if importdata
    FDTR_MAIN_IMPORT_vH4; % Input: tdelay, datadir, datain, mid, sigfit, 
    ...
    FDTR_MAIN_FIT_vH4; % Uses signal_data and offset arrays to execute fit,
    ...
    % Assign f, deltaR, ratio models to the model reference "data"
...
else xvals = f_data*1e-6; end % save in MHz
