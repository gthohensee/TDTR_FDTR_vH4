function [kErr_perc, kErr_abs, ErrSummary_perc, ErrSummary_abs] = ...
    errorbars_vH4(XguessIJ,datparams,sysparams,calparams,matparams,Tparams,options)
%errorbars_vH4 - calculates error bars for the Xijc fit parameters
%based on uncertainties in other thermal/system parameters. Sequentially
%calls TDTR_FIT_vH4.m to calculate how much the fitted Xsol changes
...
%specified by errorbar_conditions_vH4.m script.
%
% Inputs:
...
%
% Other m-files required: TDTR_FIT_vH4.m, TDTR_REFL_vH4.m,
%                         TDTR_TDEP_vH4.m, SS_Heating_vH4.m,
%                         TDTR_TEMP_vH4.m.
% Subfunctions: none
% MAT-files required: none
%
% See also: errorbars_TTM_vH4.m, senseplot_vH4.m
...
%                   11-Jul-2016 - vH4. No changes. NOT TESTED ON FDTR.
%------------- BEGIN CODE --------------
%% unpack inputs
INITIALIZE_CELLPARAMS_vH4; % unpacks/cleans the five cellparams

%% Initialize variables and uncertainties
Xij = XguessIJ(:,2:3);
Xguess = XguessIJ(:,1);
errorbar_conditions_vH4; % default uncertainties
...
Xsol=fminsearch(@(X) TDTR_FIT_vH4(X,Xij,datparams,sysparams,calparams,...
...
for i = 1:4
    for j=1:nl
        ...
        Xsoltemp=fminsearch(@(X) TDTR_FIT_vH4(X,Xij,datparams,sysparams,calparams,...
                              matparams,Tparams),Xguess,options);
