function [S_LCTE,S_sys] = senseplot_offset_vH4(datparams,sysparams, calparams, matparams, Tparams)
...
%------------- BEGIN CODE --------------
%%
INITIALIZE_CELLPARAMS_vH4; % unpacks/cleans cellparams (the five inputs)
...
if ~doughnut % conventional TDTR
    [deltaR_model,ratio_model]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,0);
    switch sigfit
        case 1 % V(in) fit
    ...
    [deltaR_model,ratio_model]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,xvect);
...
for i = 1:4
    for j=1:nl
        ...
        [deltaR_temp{i,j},ratio_temp{i,j}] = TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,offset);
...
for i = sys_consider
    ...
    [~,I] = min(abs(offset));
    xvect = offset(I:length(offset)); % this will be 0 if conventional TDTR
    [temp_deltaR,temp_ratio]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,offset);
    if ~doughnut
        switch sigfit
            case 1
...
if is_timedelay,
    xlabel('time delay (ns)','FontSize',16);
else
...
if is_timedelay
    if ~doughnut
        switch sigfit
            ...
            case 3, ylabel(sprintf('Sensitivity:  dlog[nAmp @ %0.0f ps]/dlogX',tdelay(Zind)*1e12), 'FontSize',16);
            otherwise, ylabel('Sensitivity:  dlogR/dlogX', 'FontSize',16);
        end
    else % by_FWHM = TRUE
        ylabel(sprintf('Sensitivity:  dlog[V(out) FWHM]/dlogX'), 'FontSize',16);
    end
    set(gca, 'XTick', [1e-11, 2e-11, 5e-11, 1e-10,2e-10,5e-10,1e-9,2e-9,5e-9,1e-8, 2e-8]);
    set(gca, 'XTickLabel', [0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20]);
    set(gca, 'XMinorTick','off')
    axis([100e-12 20e-9 -2 2]);
else % beam offset
