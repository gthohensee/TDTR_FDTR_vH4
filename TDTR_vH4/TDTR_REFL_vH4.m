function [deltaR,ratio]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,...
...
%
% Syntax:  [deltaR,ratio]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,...
...
%    spinning - BOOLEAN: Is the sample moving under the beam? Then the probe pulses
%               will land offset (like for doughnut) from the pump position
...
%                   17-Feb-2015 - vH3.
%                   4-Sep-2015  - introduce "spinning" condition, as
%                                 attempt to properly model SJ2 TDTR.
%                                 See Sushi Train scripts.
%                   11-Jul-2016 - vH4.
%------------- BEGIN CODE --------------
%% check input
...
if PL(4) < 7, spinning = 0; else spinning = matparams{7}; end

if nargin < 7
    offset = 0; 
    if doughnut || spinning, warning('Assuming zero offset in REFL_vH4.'); end 
end
...
if nargin < 3, error('Insufficient parameters for TDTR_REFL_vH4.'); end
...
for jj = 1:length(offset)
    ...
    if spinning
        %tdelay = tdelay_full(jj);
        if length(r_pump) == 1
            r_pump = r_pump * ones(size(offset)); % necessary for matrix sizing in TDTR_TEMP_SUSHITRAIN.
        end
        xo = offset;
    end
    ...
    if BI
        ...
        switch intscheme
            case 0 % Legendre-Gauss, allows variable pump spot size.
                ...
                if doughnut
                    I1u = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump,xo);
                    I2u = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump,xo);
                else
                    if spinning
                        I1u = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump,xo);
                        I2u = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump,xo);
                    else
                        I1u = TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump);
                        I2u = TDTR_TEMP_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump);
                    end
                end
                ...
                if doughnut
                    I1d = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump,xo);
                    I2d = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump,xo);
                else
                    if spinning
                        I1d = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump,xo);
                        I2d = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump,xo);
                    else
                        I1d = TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump);
                        I2d = TDTR_TEMP_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump);
                    end
                end
            ...
            case 1 % rombint [had a +/-f typo here before Nov. 13, 2014.]
                ...
                if doughnut
                    dT1u=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    dT2u=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    dT1d=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    dT2d=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                else
                    if spinning
                        dT1u=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                        dT2u=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                        dT1d=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                        dT2d=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    else
                        dT1u=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTEu,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                        dT2u=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep-f,LCTEu,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                        dT1d=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                        dT2d=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                    end
                end
        ...
        switch intscheme
            case 0 % Legendre-Gauss [compatible with time delay dependent spot size]
                ...
                if doughnut % TRUE if this is a beam offset measurement
                    I1 = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,xo);
                    I2 = TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep-f,LCTE,r_pump,r_probe,A_pump,xo);
                else
                    if spinning
                        I1 = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,xo);
                        I2 = TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep-f,LCTE,r_pump,r_probe,A_pump,xo);
                    else
                        I1 = TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump);
                        I2 = TDTR_TEMP_vH4(kvect,mvect/tau_rep-f,LCTE,r_pump,r_probe,A_pump);
                    end
                end
            ...
            case 1 % rombint
                if length(r_pump)>1, error('Sorry, variable pump spot size is not yet compatible with rombint.'); end
                if doughnut
                    dT1=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    dT2=rombint_VV3(@(kvect) TDTR_TEMP_DOUGHNUT_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                else
                    if spinning
                        dT1=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                        dT2=rombint_VV3(@(kvect) TDTR_TEMP_SUSHITRAIN_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump,offset),0,kmax,length(mvect));
                    else
                        dT1=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                        dT2=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                    end
                end
            case 2 % Simpson
                if length(r_pump)>1, error('Sorry, variable pump spot size is not yet compatible with Simpson int.'); end
    ...
    NNt = ones(length(tdelay),1); % because Nt has been co-opted as the "number of pump spot size elements", which equals length(tdelay).
    ...
    if spinning 
        % deltaR and ratio are square matrices, since offset and 
        % tdelay_full have same length; here tdelay is a scalar.
        deltaR =deltaRm.*exp(ii*2*pi*f*tdelay); %Reflectance Fluxation (Complex)
        ratio =-real(deltaR)./imag(deltaR);
    else
        % here tdelay is the full tdelay array; either jj = 1 and tdelay is
        % an array (conventional TDTR) or jj varies across offset and 
        % tdelay is scalar (beam offset).
        
        % An exponential exp(ii*2*pi*f*tdelay) is applied because pump is on the delay line.
        % If *probe* is on delay stage, this term is discarded? (Cahill RSI 2004)
        deltaR(:,jj) =deltaRm.*exp(ii*2*pi*f*tdelay); %Reflectance Fluxation (Complex)
        ratio(:,jj) =-real(deltaR(:,jj))./imag(deltaR(:,jj));
    end
    
    if spinning
        break; % only one iteration of code inside for loop if spinning is true, because SUSHITRAIN is parallelized.
    end 
