function [S_LCTE,S_sys,xvar,SS_LCTE,SS_sys] = parametric_senseplot_vH4_notN(Xij,...
    SYS_i, xvar,datparams,sysparams, calparams, matparams, Tparams,LCTE_sens_consider,sys_consider)
%parametric_senseplot_vH4_notN - Calculates sensitivity plots dlogR/dlogX for 
...
%
% See also: errorbars_vH4.m, senseplot_offset_vH4.m
...
%     return;
    ...
    for i = 1:4
        for j=1:nl
            ...
            [deltaR_temp{i,j},ratio_temp{i,j}] = TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,xvect);
    ...
    for i = 1:6
        ...
        if i ~= 6
            ...
            [~,I] = min(abs(offset));
            xvect = offset(I:length(offset)); % this will be 0 if conventional TDTR
            [temp_deltaR,temp_ratio]=TDTR_REFL_vH4(tdelay,matparams,sysparams,A_pump,intscheme,nnodes,xvect);
...
% S_LCTE{i,j} and S_sys{i}, analogous to output of senseplot_offest_vH4.
...
if ~doughnut % conventional TDTR
    ...
    for t_index = 1:length(tdelay)
        ...
        nVout_model(t_index,:) = sign(Vout_model(I)) * Vout_model' / norm; 
...
if ~doughnut
    ...
    for t_index = 1:length(tdelay) % for loop is crude hack for non-scalar tdelay.
        ...
        nVout_temp = sign(Vout_temp(I)) * Vout_temp / norm;
