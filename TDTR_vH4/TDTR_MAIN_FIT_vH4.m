...
if manualfit
    if ~doughnut
        switch sigfit
            ...
            case 3, fprintf('Manual fitting to Amp(t) normalized near %i ps...\n',tdelay(Zind)*1e12);
            case 4, fprintf('Manual fitting to Amp(t) normalized near %i ps...\n',tdelay(Zind)*1e12);
    ...
    tic
    Xsol = fminsearch(@(X) TDTR_FIT_vH4(X, Xij, datparams,...
                           sysparams, calparams, matparams, Tparams),...
                           Xguess,options);

    % fminsearch doesn't output anything but Xsol, so get the rest here.                
    [Z,deltaR_model,ratio_model,LCTE,T_adj]=...
        TDTR_FIT_vH4(Xsol, Xij,datparams,sysparams,calparams,matparams,Tparams);
    toc
end
