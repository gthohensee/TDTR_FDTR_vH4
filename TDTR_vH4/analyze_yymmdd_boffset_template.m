...
tagline = 'negboff'; % a shared prefix for your series of TDTR beam offset data files.
...
f_list = 1.77 * ones(1,nfiles); % pump modulation frequency
...
for ii = 2:nfiles
   ...
   switch r_list(ii) % set pump 1/e^2 intensity focused radius, in meters.
        ...
        case 20, r_pump = 3.7e-6; tc = 0.70; % TDTR-2
   ...
   % The beam offset data generally isn't centered exactly at x = 0 offset.
   % Use Gaussian fitting to find the midpoint of your data, and apply
   % these mid parameters to correct the offset so it lines up with the
   % model (which is of course always centered at x = 0).
   mids = [-0.484
           -0.363
            0.088];
   mid = mids(ii);
   
   TDTR_MAIN_vH4;  % Now that all system and material parameters areoffse
